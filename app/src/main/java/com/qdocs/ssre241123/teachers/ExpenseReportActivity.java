...
/**
 * Activity for Expense Report
 * Shows all expense records with optional filters
 */
public class ExpenseReportActivity extends AppCompatActivity {

    private static final String TAG = "ExpenseReport";
    ...
    private void fetchExpenseReport() {
        Log.d(TAG, "=== Fetching Expense Report ===");

        // Get base URL from shared preferences
        String baseUrl = Utility.getSharedPreferences(getApplicationContext(), "apiUrl");
        String url = baseUrl + Constants.expenseReportFilterUrl;
            ...
            if (status == 1) {
                ...
                if (dataArray != null) {
                    ...
                    for (int i = 0; i < dataArray.length(); i++) {
                        ...
                        expense.setId(dataObj.optString("id", ""));
                        expense.setName(dataObj.optString("name", ""));
                        expense.setInvoiceNo(dataObj.optString("invoice_no", ""));
                        expense.setDate(dataObj.optString("date", ""));
                        expense.setAmount(dataObj.optString("amount", "0"));
                        expense.setExpCategory(dataObj.optString("exp_category", ""));
                        expense.setExpHeadId(dataObj.optString("exp_head_id", ""));
                        expense.setNote(dataObj.optString("note", ""));
                        expense.setDocuments(dataObj.optString("documents", ""));

                        expenseList.add(expense);

                        if (i == 0) {
                            Log.d(TAG, "First Expense - Name: " + expense.getName());
                            Log.d(TAG, "First Expense - Amount: " + expense.getAmount());
                        }
                    }
                }
                ...
                if (!expenseList.isEmpty()) {
                    ...
                    showNoData();
                    Toast.makeText(this, "No expense records found for the selected period",
