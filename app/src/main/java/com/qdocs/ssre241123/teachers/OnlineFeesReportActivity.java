...
import com.qdocs.ssre241123.adapters.OnlineFeesReportAdapter;
...
/**
 * Activity for Online Fees Collection Report
 * Shows fees paid through online payment gateways
 */
public class OnlineFeesReportActivity extends AppCompatActivity {

    private static final String TAG = "OnlineFeesReport";
    ...
    // Data
    private OnlineFeesReportAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_online_fees_report);
    ...
    private void setupRecyclerView() {
        ...
        adapter = new OnlineFeesReportAdapter(this, feesList);
    ...
    private void generateReport() {
        ...
        showLoading();
        fetchOnlineFeesReport();
    }

    private void fetchOnlineFeesReport() {
        Log.d(TAG, "=== Fetching Online Fees Report ===");

        // Get base URL from shared preferences
        String baseUrl = Utility.getSharedPreferences(getApplicationContext(), "apiUrl");
        String url = baseUrl + Constants.onlineFeesReportFilterUrl;
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, url,
                response -> {
                    ...
                    hideLoading();
                    parseOnlineFeesReportResponse(response);
                },
                error -> {
                    Log.e(TAG, "=== API Error ===");
    ...
    private void parseOnlineFeesReportResponse(String response) {
            ...
            if (status == 1) {
                ...
                if (dataArray != null) {
                    ...
                    for (int i = 0; i < dataArray.length(); i++) {
                        ...
                        OnlineFeesReportModel fee = new OnlineFeesReportModel();
                        fee.setId(dataObj.optString("id", ""));
                        fee.setStudentId(dataObj.optString("student_id", ""));
                        fee.setAdmissionNo(dataObj.optString("admission_no", ""));
                        fee.setStudentName(dataObj.optString("student_name", ""));
                        fee.setClassName(dataObj.optString("class", ""));
                        fee.setSectionName(dataObj.optString("section", ""));
                        fee.setFeeGroup(dataObj.optString("fee_group", ""));
                        fee.setFeeType(dataObj.optString("fee_type", ""));
                        fee.setFeeCode(dataObj.optString("fee_code", ""));
                        fee.setAmount(dataObj.optString("amount", "0"));
                        fee.setPaymentDate(dataObj.optString("payment_date", ""));
                        fee.setPaymentMode(dataObj.optString("payment_mode", ""));

                        feesList.add(fee);

                        if (i == 0) {
                            Log.d(TAG, "First Fee - Student: " + fee.getStudentName());
                            Log.d(TAG, "First Fee - Amount: " + fee.getAmount());
                        }
                    }
                }

                Log.d(TAG, "Fees list size: " + feesList.size());
                ...
                if (!feesList.isEmpty()) {
                    ...
                    String successMessage = "Found " + feesList.size() + " online fee record(s)";
                    Toast.makeText(this, successMessage, Toast.LENGTH_SHORT).show();
                } else {
                    Log.d(TAG, "No data in response");
                    showNoData();
                    Toast.makeText(this, "No online fees found for the selected period",
